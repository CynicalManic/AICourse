...
MyPathFinder_DRS::MyPathFinder_DRS(SDL_Renderer* renderer, TileType map[kMapWidth][kMapHeight])
{
...
MyPathFinder_DRS::~MyPathFinder_DRS()
{
}

//--------------------------------------------------------------------------------------------------

bool MyPathFinder_DRS::FindPath(const Vector2D startPosition, const Vector2D endPosition)
{
...
bool MyPathFinder_DRS::IsCellValid(Vector2D currentPosition)
{
...
bool MyPathFinder_DRS::Search(Vector2D currentPosition, Vector2D targetPosition, bool orderOfRotation, vector<Vector2D>* path)
{
...
vector<Vector2D>* MyPathFinder_DRS::EnhancePath(vector<Vector2D>* tempPathData)
{
...
vector<Vector2D> MyPathFinder_DRS::RefinePath(vector<Vector2D>* pathDataEnhanced)
{
...
void MyPathFinder_DRS::SetMaxCost()
{
...
bool MyPathFinder_DRS::IsPositionInMapBounds(Vector2D position)
{
...
bool MyPathFinder_DRS::IsMoveAllowed(Vector2D fromPos, Vector2D toPos)
{
...
void MyPathFinder_DRS::OutputCosts(Vector2D startPosition, Vector2D endPosition)
{
...
void MyPathFinder_DRS::ResetCostArray()
{
...
void MyPathFinder_DRS::ResetCollisionMap()
{
	for(int x = 0; x < kMapWidth; x++)
	{
		for(int y = 0; y < kMapHeight; y++)
...
void MyPathFinder_DRS::ResetCostArrayAndCollisionMap()
{
	//Set all values to zero for to ensure a fresh search.
	for(int x = 0; x < kMapWidth; x++)
	{
